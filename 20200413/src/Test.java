 ...
 * 小青蛙初始在(0,0)位置,地下迷宫的出口在(0,m-1)(保证这两个位置都是1,并且保证一定有起点到终点可达的路径),小青蛙在迷宫中水平移动一个单位距离需要消耗1点体力值,
    ...
    public static  void runMap(int x, int y, int energy) {
        if (energy < 0 || x<0 || y<0 || x>=n || y>= m || map[x][y] != 1) {
            ...
            if (x == 0 && y == m - 1) {
                ...
                map[x][y] = 1;  //把堵住的路释放
                linkedlist.removeLast(); //回溯回退
                flag = true; return;
            }

            runMap(x, y+1, energy-1); //向右
            runMap(x-1, y, energy-3); //向上
            runMap(x+1, y, energy); //向下
            runMap(x, y-1, energy-1); //向左
            map[x][y] = 1;   //把堵住的路释放
            linkedlist.removeLast(); //回溯回退
        }
    }
